...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def ValueIteration(self):
        for i in range(self.iterations):
            ...
            for state in self.mdp.getStates():
                ...
                QValue = self.computeQValueFromValues(state, 
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for nextState, proba in self.mdp.getTransitionStatesAndProbs(state, action):
            QValue += proba * (self.mdp.getReward(state, action, nextState) + self.discount * self.getValue(nextState))
    ...
    def computeActionFromValues(self, state):
        ...
        for i in self.mdp.getPossibleActions(state):
             actionQValues[i] = self.computeQValueFromValues(state, i)
        #util.raiseNotDefined()
